...
```{r reg-plot-1, cache=TRUE, echo=T, warning=F, message=FALSE}
...
```{r reg-plot-2, cache=TRUE, echo=T, warning=F, message=FALSE}
piaac %>% 
  ggplot(aes(x = numeracy, y = sissetulek))+
...
```{r reg-plot-3, cache=TRUE, echo=T, warning=F, message=FALSE}
piaac %>% 
  ggplot(aes(x = numeracy, y = sissetulek))+
  geom_point(size = 0.3, alpha = 0.2)+
...
```{r reg-plot-4, fig.cap = "Prognoosimine y väärtust kui x on 200", cache=TRUE, echo=F, warning=F}
...
```{r reg-plot-5, fig.cap = "Regressiooni jäägid", cache=TRUE, echo=F, warning=F}
# coefs
b0 <- 38.93
b1 <- 0.85
sigma <- 27
...
```{r ss, fig.cap = "Variatsiivsuse jagunemine", cache=TRUE, echo=F, warning=F}
# coefs
b0 <- 30
b1 <- 7
sigma <- 9
...
```{r reg-plot-6, cache=TRUE, echo=F, warning=F}
piaac %>% 
  group_by(sugu) %>% 
...
```{r reg-plot-7, cache=TRUE, echo=T, warning=F, message=FALSE}
piaac %>% 
...
```{r reg-plot-8, cache=TRUE, echo=T, warning=F, message=FALSE}
...
```{r anscombe, fig.cap = "Anscombe kvartett", fig.height=4.5, cache=TRUE, echo=F, warning=F, fig.show='hold'}
