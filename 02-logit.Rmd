...
- *(Intercept)* - vabaliiget tõlgendame kui referentsgrupi (antud juhul naiste) šansse ellu jääda. Seega naise šanss Titanicul ellu jääda oli $`r odds_naised`$, ehk siis iga hukkunud mehe kohta jäi ellu $`r odds_naised`$ naist, või vastupidi $1 \div `r odds_naised` = `r round(1/odds_naised, 2)`$, iga ellujäänud mehe kohta hukkus $`r 1/odds_naised`$ naist. Saame välja arvutada ka naiste ellujäämise tõenäosuse:
