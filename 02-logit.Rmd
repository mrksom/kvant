...
- Vabaliiget tõlgendame kui referentsgrupi (antud juhul naiste) šansse ellu jääda. Seega naise šanss Titanicul ellu jääda oli $`r odds_naised`$, ehk siis iga hukkunud mehe kohta jäi ellu $`r odds_naised`$ naist, või vastupidi $1 / `r odds_naised` = `r round(1/odds_naised, 2)`$, iga ellujäänud mehe kohta hukkus $`r 1/odds_naised`$ naist. Saame välja arvutada ka naiste ellujäämise tõenäosuse:

$$\pi=\frac{\text{šansid}}{1+\text{šansid}} = \frac{`r odds_naised`}{1+`r odds_naised`} = `r round(odds_naised/(1+odds_naised), 2)`$$  

- Meeste puhul tõlgendame šansside suhet. Ehk kui palju muudab meheksolemine võrreldes naistega ellujäämise šansse. Tuleb välja, et ligi $`r or_mehed`$ korda. Seega meeste šansid ellu jääda olid $`r or_mehed` \times `r odds_naised` = `r odds_mehed`$. Iga hukkunud naise kohta jäi `r odds_mehed` meest ellu. Meeste ellujäämise tõenäosus oli:
...
Täiskasvanuks olemine mõnevõrra tõstis ellujäämise tõenäosust, kuid seda ainult naiste puhul (soo ja vanuse interaktsioon on negatiivne ning meeste vanuse koefitsient on `r b2` + `r b3` = `r b2 + b3`). Tulemuste tõlgendamiseks võtame jälle koefitsientidest eksponendi:
...
```
Saame järeldada, et meeste tõenäosus ellu jääda oli $`r abs(round(summary(margins(mudel9, variables = 'sex'))[, 2]*100, 0))`\%$ madalam kui naistel.
